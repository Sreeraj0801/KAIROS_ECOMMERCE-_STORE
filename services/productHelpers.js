...
module.exports = {
    ...
    findBanner: (id) => {
        return new Promise(async (resolve, reject) => {
            banner = await db.get().collection(collections.BANNER).findOne({ _id: objectId(id) });
            resolve(banner);
        })
    },
    ...
    addproductOffer: (details) => {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (products.offerPercentage) {
                ...
                await db.get().collection(collections.PRODUCT).updateMany({ _id: details.id }, {
                    $set: {
                        oldPrice: oldPrice,
                        price: offerPrice,
    ...
    removeProductOffer: (id) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (product.oldPrice) {
                ...
                await db.get().collection(collections.PRODUCT).updateOne({ _id: objectId(id) }, {
                    $unset: {
                        offerPercentage: 1,
                        oldPrice: 1
                    }
    ...
    checkCoupon: () => {
        return new Promise((resolve, reject) => {
            ...
            let Year = date.getFullYear();
            let finaldate = `${Year}-${Month}-${Day}`
            db.get().collection(collections.COUPON).update({ 'details.expiryDate': finaldate }, {
                $set: {
